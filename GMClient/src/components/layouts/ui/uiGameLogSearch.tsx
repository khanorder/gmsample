...
interface GameLogSearchProps {
    onChange: (type: Defines.GameLogSearchType, value: string) => void;
    onSubmit: (type: Defines.GameLogSearchType, value: string) => void;
    ignoreSearchType?: Defines.GameLogSearchType[];
    size?: "small" | "medium";
    label?: string;
}

function GameLogSearch ({ onChange, onSubmit, ignoreSearchType, size, label }: GameLogSearchProps) {
    const [searchType, setSearchType] = useState<Defines.GameLogSearchType>(Defines.GameLogSearchType.EventGroupID);
    const [searchValue, setSearchValue] = useState<string>("");

    const onChangeSearchType = useCallback((e: SelectChangeEvent<unknown>) => {
        let value: Defines.GameLogSearchType = 0;
    ...
    const searchTypeSelectList = () => {
        ...
        let enumSize = 0;
        if (0 < Object.values(Defines.GameLogSearchType).length)
            enumSize = Object.values(Defines.GameLogSearchType).length / 2;
        ...
        for (let i = 0; i < enumSize; i++) {
            ...
            list.push(<MenuItem key={i} value={i}>{Defines.GameLogSearchType[i].replaceAll(/(?<=.{3})./g, '*')}</MenuItem>);
        }
