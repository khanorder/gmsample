...
import { insertEventMailReq } from 'src/ngel/data/hubs/GMServer/reducer';
...
const EventMailInsertForm = (): ReactElement => {
    ...
    useEffect(() => {
        if (firstRender.current) {
            ...
            setDatas(prev => {
                prev.isNewData = true;
                prev.MailType = EMailType.OpMail;
                prev.ExpireTime = 1;
                return deepmerge([], prev);
            });
        }

    }, [firstRender, tables, setItemLimit, setDatas, itemLimitDefine]);
    ...
    const saveData = useCallback(async () => {
        ...
        dispatch(insertEventMailReq({ eventMail: datasTrim }));
    }, [datas, dispatch, mailExpireTime]);

    const selectRewardType = useCallback((rewardIndex: number): ReactElement => {
        if (!(datas.RewardList[rewardIndex]) || 1 > datas?.RewardList.length) return <></>;
        ...
        return (
            <ListItem key={`select-li-${rewardIndex}`}>
                <FormControl key={`select-form1-${rewardIndex}`} fullWidth size="small">
                    <InputLabel key={`select-label-${rewardIndex}`} id={`label-rewardType${rewardIndex}`}>{`보상종류${rewardIndex}`}</InputLabel>
                    <Select key={`select-rewardType-${rewardIndex}`} labelId={`select-rewardType${rewardIndex}`} id={`select-rewardType${rewardIndex}`} value={datas.RewardList[rewardIndex].RewardType} onChange={(e, v) => onChangeRewardType(e, v as ERewardType, rewardIndex)} label={`보상종류${rewardIndex}`}>
                ...
                <FormControl key={`select-form2-${rewardIndex}`} fullWidth sx={{ marginLeft: 1 }}>
                    <Autocomplete key={`select-auto-${rewardIndex}`} id={`item_${(rewardIndex)}`} options={opts} size='small' onChange={(e, v, r) => onChangeItem(e, v as ItemInput, rewardIndex)} getOptionLabel={(option) => (option as ItemInput).name.toString().replaceAll(/(?<=.{2})./g, '*')} isOptionEqualToValue={(option, value) => (option as ItemInput).id == (value as ItemInput).id} value={item} renderInput={(params) => <TextField {...params} label={`아이템 ${(rewardIndex)}`} />} />
                </FormControl>
                <FormControl key={`select-form3-${rewardIndex}`} fullWidth sx={{ marginLeft: 1 }}>
                    <TextField key={`select-text-${rewardIndex}`} className={commonUIStyles.input} variant='outlined' value={datas.RewardList[rewardIndex].RewardCount}
    ...
    const contents = useCallback((): ReactElement => {
        ...
        for (let i = 0; i < datas.RewardList.length; i++) {
            itemSelectList.push(selectRewardType(i));
        }
        ...
        <Box sx={{ mt: 5, width: '100%' }}>
            ...
            <TableContainer component={Paper} elevation={4} sx={{ marginBottom: 10 }}>
                <Toolbar variant='dense' className={commonUIStyles.toolBar}>
                    <Typography variant='h6' noWrap sx={{ flexGrow: 1 }}>이벤트 우편 발송 (신규 데이터)</Typography>
                </Toolbar>
                <Table>
                    {contents()}
                </Table>
...
export default EventMailInsertForm;