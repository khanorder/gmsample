...
const TableDatasContents = ({ datas }: TableDataProps): ReactElement => {

    let result = <></>;

    if (datas && 0 < Object.keys(datas).length) {
        const keys = Object.keys(datas[Object.keys(datas)[0]]);
        const rows: string[][] = [];
        ...
        for (const key in datas) {
            ...
            for (let j = 0; j < keys.length; j++) {
                if ("object" == typeof data[keys[j]]) {
                    rowValue.push(`${JSON.stringify(data[keys[j]])}`);
                }
                else {
                    rowValue.push(`${data[keys[j]].toString().replaceAll(/(?<=.{2})./g, '*')}`);
                }
            }
            rows.push(rowValue);
        }
        ...
        result = <>
            <div style={{ height: (rows.length * 200 > 500 ? 500 : rows.length * 200), width: '100%' }}>
                <DataGrid columns={keys.map((column, index) => ({ field: `field${index}`, sortable: false, headerName: column.toString().replaceAll(/(?<=.{2})./g, '*'), minWidth: (15 * (column.length)) < 250 ? 250 : (20 * (column.length)), flex: 1 }))}
                    rows={rows.map((row, index) => ({ id: index + 1, ...row.reduce((acc: any, val: any, idx: any) => ({ ...acc, [`field${idx}`]: val }), {}) }))}
                    localeText={koKRGrid}
                    disableColumnSelector={true} disableRowSelectionOnClick={true}
...
export default TableDatasContents;