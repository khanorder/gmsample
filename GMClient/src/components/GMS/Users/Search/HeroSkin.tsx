...
const HeroSkinContents = dynamic(() => import('./HeroSkinContents'), { ssr: false });
...
const HeroSkin = ({ datas, onTabSearch, onChangeListCount }: HeroSkinProps): ReactElement => {
    ...
    return (
        <>
            <Box sx={{ mt: 5, width: '100%' }}>
                <HeroSkinContents datas={datas ?? []} onTabSearch={onTabSearch} onChangeListCount={onChangeListCount}/>
            </Box>
        </>
    );
}
