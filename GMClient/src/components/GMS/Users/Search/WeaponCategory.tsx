...
const WeaponCategoryContents = dynamic(() => import('./WeaponCategoryContents'), { ssr: false });
...
export interface WeaponCategoryProps {
    weaponCategories: Models.WeaponCategory[]|null;
}

const WeaponCategory = ({ weaponCategories }: WeaponCategoryProps): ReactElement => {
            ...
            <Box sx={{ mt: 5, width: '100%' }}>
                <TableContainer component={Paper} elevation={4} sx={{ maxHeight: 500 }}>
                    ...
                    <Table className={commonUIStyles.ellipsisTable} stickyHeader aria-label="sticky table">
                        <WeaponCategoryContents datas={weaponCategories ?? []} />
                    </Table>
                </TableContainer>
            </Box>
        </>
