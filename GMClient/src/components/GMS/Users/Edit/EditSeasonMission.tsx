...
const SeasonMissionContents = dynamic(() => import('./EditSeasonMissionContents'), { ssr: false });
...
const SeasonMission = ({ userAccount, tabIndex, datas, searchDetail, tabContentChanged, onChangePageListCount }: SeasonMissionProps): ReactElement => {
    ...
    const onReload = useCallback(async () => {
        if (!seasonMissions) return;

        const changedDatas = seasonMissions.totalItems.filter(_ => _.isChanged);
    ...
    }, [loadDatas, tabContentChanged, seasonMissions]);
    ...
    const onSave = useCallback(async () => {
        ...
        if (0 < changedDatas.length) {
            const response = await GameAPI.SaveSeasonMissionsAsync({ seasonMissions: changedDatas });
            if (!response) {
                alert(`오류!`);
                return;
            }
            ...
            <Box sx={{ mt: 5, width: '100%' }}>
                ...
                <TableContainer component={Paper} elevation={4} sx={{ maxHeight: 700 }}>
                    <Toolbar variant='dense' className={commonUIStyles.toolBar}>
                        <Typography variant='h6'>시즌미션</Typography>
                    </Toolbar>
                    <Table className={commonUIStyles.ellipsisTable} stickyHeader aria-label="sticky table">
                        <SeasonMissionContents userAccount={userAccount} datas={seasonMissions} />
                    </Table>
                </TableContainer>
            </Box>
            <Paging datas={seasonMissions} />
        </>
    );
}
