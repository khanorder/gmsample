...
namespace NGEL.Data.Tables
{
    public class ItemTableService
    {
        ...
        private string csvFileName = "tb_Item";
        private readonly ILogger<ItemTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.Item> _datas { get; set; } = new List<NGEL.Data.Tables.Models.Item>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.Item> datas { get => _datas; }
        private object lockObject = new object();

        public ItemTableService(ILogger<ItemTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                while (csv.Read())
                {
                    ...
                    var WeaponType = csv.GetField("WeaponType") ?? "";
                    if (null == WeaponType)
                    {
                        WeaponType = "";
                        _logger.LogError($"{tableName}({rowIndex}): Member WeaponType is null.");
                    }
                    else if (string.IsNullOrWhiteSpace(WeaponType))
                    {
                        WeaponType = "";
                    }
                    ...
                    var data = new NGEL.Data.Tables.Models.Item
                    {
                        ID = ID,
                        Name = Name,
                        ItemType = ItemType,
