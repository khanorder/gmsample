...
{
    public class WorldMapDataTableService
    {
        private string tableName = "WorldMapDataTable";
        private string csvFileName = "tb_WorldMap";
        private readonly ILogger<WorldMapDataTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.WorldMapData> _datas { get; set; } = new List<NGEL.Data.Tables.Models.WorldMapData>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.WorldMapData> datas { get => _datas; }
        private object lockObject = new object();

        public WorldMapDataTableService(ILogger<WorldMapDataTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Context.RegisterClassMap<WorldMapDataMap>();
                csv.Read();
                csv.ReadHeader();
                Int64 rowIndex = 0;
                ...
                while (csv.Read())
                {
                    ...
                    var WorldmapPopTitle = csv.GetField("WorldmapPopTitle") ?? "";
                    if (null == WorldmapPopTitle)
                    {
                        WorldmapPopTitle = "";
                        _logger.LogError($"{tableName}({rowIndex}): Member WorldmapPopTitle is null.");
                    }
                    else if (string.IsNullOrWhiteSpace(WorldmapPopTitle))
                    {
                        WorldmapPopTitle = "";
                    }

                    var data = new NGEL.Data.Tables.Models.WorldMapData
                    {
                        ID = ID,
                        WorldmapPopTitle = WorldmapPopTitle,
                    };
                    _datas.Add(data);
                    rowIndex++;
                }
