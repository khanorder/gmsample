...
{
    public class AttendanceDataTableService
    {
        private string tableName = "AttendanceDataTable";
        private string csvFileName = "tb_Attendance";
        private readonly ILogger<AttendanceDataTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.AttendanceData> _datas { get; set; } = new List<NGEL.Data.Tables.Models.AttendanceData>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.AttendanceData> datas { get => _datas; }
        private object lockObject = new object();

        public AttendanceDataTableService(ILogger<AttendanceDataTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Context.RegisterClassMap<AttendanceDataMap>();
                csv.Read();
                csv.ReadHeader();
                Int64 rowIndex = 0;
                ...
                while (csv.Read())
                {
                    ...
                    try
                    {
                        var fieldRewardTotalCount = csv.GetField("RewardTotalCount");
                        if (null != fieldRewardTotalCount)
                            RewardTotalCount = int.Parse(fieldRewardTotalCount);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError($"{tableName}({rowIndex}): Occurred Error to parse RewardTotalCount.");
                        _logger.LogError(ex.Message);
                        _logger.LogError(ex.StackTrace);
                    }
                    ...
                    var data = new NGEL.Data.Tables.Models.AttendanceData
                    {
                        ID = ID,
