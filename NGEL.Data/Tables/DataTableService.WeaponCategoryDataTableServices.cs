...
{
    public class WeaponCategoryDataTableService
    {
        private string tableName = "WeaponCategoryDataTable";
        private string csvFileName = "tb_WeaponCategory";
        private readonly ILogger<WeaponCategoryDataTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.WeaponCategoryData> _datas { get; set; } = new List<NGEL.Data.Tables.Models.WeaponCategoryData>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.WeaponCategoryData> datas { get => _datas; }
        private object lockObject = new object();

        public WeaponCategoryDataTableService(ILogger<WeaponCategoryDataTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Context.RegisterClassMap<WeaponCategoryDataMap>();
                csv.Read();
                csv.ReadHeader();
                Int64 rowIndex = 0;
                ...
                while (csv.Read())
                {
                    ...
                    EWeaponCategory WeaponCategory = 0;
                    try
                    {
                        var fieldWeaponCategory = csv.GetField("WeaponCategory");
                        if (null != fieldWeaponCategory)
                            WeaponCategory = Enum.Parse<EWeaponCategory>(fieldWeaponCategory, true);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError($"{tableName}({rowIndex}): Occurred Error to parse WeaponCategory.");
                        _logger.LogError(ex.Message);
                        _logger.LogError(ex.StackTrace);
                    }
                    ...
                    var data = new NGEL.Data.Tables.Models.WeaponCategoryData
                    {
                        ID = ID,
