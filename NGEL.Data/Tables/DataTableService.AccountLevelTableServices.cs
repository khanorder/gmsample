...
{
    public class AccountLevelTableService
    {
        private string tableName = "AccountLevelTable";
        private string csvFileName = "tb_AccountLevel";
        private readonly ILogger<AccountLevelTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.AccountLevel> _datas { get; set; } = new List<NGEL.Data.Tables.Models.AccountLevel>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.AccountLevel> datas { get => _datas; }
        private object lockObject = new object();

        public AccountLevelTableService(ILogger<AccountLevelTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Context.RegisterClassMap<AccountLevelMap>();
                csv.Read();
                csv.ReadHeader();
                Int64 rowIndex = 0;
                ...
                while (csv.Read())
                {
                    ...
                    try
                    {
                        var fieldExperience = csv.GetField("Experience");
                        if (null != fieldExperience)
                            Experience = Int64.Parse(fieldExperience);
                    }
                    catch (Exception ex)
                    {
                        _logger.LogError($"{tableName}({rowIndex}): Occurred Error to parse Experience.");
                        _logger.LogError(ex.Message);
                        _logger.LogError(ex.StackTrace);
                    }

                    var data = new NGEL.Data.Tables.Models.AccountLevel
                    {
                        ID = ID,
