...
{
    public class EntitlementDataTableService
    {
        private string tableName = "EntitlementDataTable";
        private string csvFileName = "tb_Entitlement";
        private readonly ILogger<EntitlementDataTableService> _logger;
        private double _version { get; set; } = 0;
        private List<NGEL.Data.Tables.Models.EntitlementData> _datas { get; set; } = new List<NGEL.Data.Tables.Models.EntitlementData>();
        public double version { get => _version; }
        public List<NGEL.Data.Tables.Models.EntitlementData> datas { get => _datas; }
        private object lockObject = new object();

        public EntitlementDataTableService(ILogger<EntitlementDataTableService> logger)
        {
            _logger = logger;
        }
        ...
        public void LoadTable()
        {
            ...
            using (var csv = new CsvReader(reader, config))
            {
                ...
                csv.Context.RegisterClassMap<EntitlementDataMap>();
                csv.Read();
                csv.ReadHeader();
                Int64 rowIndex = 0;
                ...
                while (csv.Read())
                {
                    ...
                    var EntitlementName = csv.GetField("EntitlementName") ?? "";
                    if (null == EntitlementName)
                    {
                        EntitlementName = "";
                        _logger.LogError($"{tableName}({rowIndex}): Member EntitlementName is null.");
                    }
                    else if (string.IsNullOrWhiteSpace(EntitlementName))
                    {
                        EntitlementName = "";
                    }
                    ...
                    var data = new NGEL.Data.Tables.Models.EntitlementData
                    {
                        ID = ID,
